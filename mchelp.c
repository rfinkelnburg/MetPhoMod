...
,
  ...
  for (i = 4; --i >= 0 && strcmp(*argv, helpkey[i]); );
  if (i >= 0)  {
    printf("%s\n", helptxt[i+1]);
    return;
  }
  for (i = NULL_SECTION; --i >= 0 && strcmp(*argv, section[i].name); );
  if (i >= 0)  {
    ...
    for (v = variable; v; v = v->next)
      if (i == v->section)
        printf("%-2c%-22s: %s\n",
           ...
           v->name, v->comment);
    if (section[i].dependson != NULL_SECTION)  printf("\nIt depends on Section(s):\n\n");
    for (i = section[i].dependson; i != NULL_SECTION; i = section[i].dependson)
      printf("%s\n", section[i].name);
    printf("\n");
    return;
  }
